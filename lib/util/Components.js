...
function componentRule(rule, context, _node) {
  ...
  const utils = {
     ...
     */
    getParentComponent: function (_node) {
      return (
        utils.getParentES6Component(_node)
        || utils.getParentES5Component(_node)
        || utils.getParentStatelessComponent(_node)
      );
    },
     ...
     * @returns {ASTNode} component node, null if we are not in a component
     */
    getParentES5Component: function (_node) {
     ...
     * @returns {ASTNode} component node, null if we are not in a component
     */
    getParentES6Component: function (_node) {
      let scope = (context.sourceCode || context).getScope(_node);
     ...
     * @returns {ASTNode} component node, null if we are not in a component
     */
    getParentStatelessComponent: function (_node) {
      // eslint-disable-next-line react/destructuring-assignment
      let scope = (context.sourceCode || context).getScope(_node);
      while (scope) {
  ...
  const detectionInstructions = {
    ...
    ClassProperty: function (_node) {
      const node = utils.getParentComponent(_node);
      if (!node) {
        return;
      }
      components.add(node, 2);
    ...
    FunctionDeclaration: function (_node) {
      const node = utils.getParentComponent(_node);
      if (!node) {
        return;
      }
