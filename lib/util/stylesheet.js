...
const astHelpers = {
  ...
  isStyleAttribute: function (node) {
    return Boolean(
      ...
      && node.name
      && node.name.name
      && astHelpers.isStyleSheetName(node.name.name)
    );
  },

  isStyleSheetName: function (value) {
    return /styles?/i.test(value);
  },
  ...
  isNodePotentialStyleReferenceFromMemberExpression: function (node) {
    if (
      node
      && node.object
  ...
  getPotentialStyleReferenceFromMemberExpression: function (node) {
    if (this.isNodePotentialStyleReferenceFromMemberExpression(node)) {
      return [node.object.name, node.property.name].join('.');
    }
  },

  isNodePotentialStyleReferenceFromMemberExpressionWithObjectName: function (node, objectName) {
    if (
      this.isNodePotentialStyleReferenceFromMemberExpression(node) &&
